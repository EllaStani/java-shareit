...
public class ItemServiceImplTest {
    ...
    void setUp() {
        ...
        itemInDto = new ItemInDto(1L, "Item", "ItemDescription", true, null);
    ...
    public void searchItems() {
        ...
        when(itemRepository.search("tion")).thenReturn(List.of(item1, item2));

        var result = itemService.searchItems(1L,"tion");
        assertNotNull(result);
        ...
        verify(itemRepository, times(1)).search("tion");
    }

    @Test
    public void searchItemsWithEmptyResult() {
        Pageable pageable = FromSizeRequest.of(0, 10, Sort.unsorted());
        when(itemRepository.search("abc")).thenReturn(Collections.emptyList());

        var result = itemService.searchItems(1L, "abc");

        assertNotNull(result);
        Assertions.assertEquals(0, result.size());
        verify(itemRepository, times(1)).search("abc");
    }
    ...
    public void saveNewItemWithoutRequest() {
        ...
        var result = itemService.saveNewItem(user1.getId(), itemInDto);
    ...
    public void saveNewItemUnknownUser() {
        ...
        Assertions.assertThrows(NotFoundException.class,
                () -> {
                    itemService.saveNewItem(100L, itemInDto);
                });
    }

    @Test
    public void updateItem() {
        ItemInDto updateItemDto = new ItemInDto();
        updateItemDto.setName("updateItem");
        updateItemDto.setDescription("updateDescription");
